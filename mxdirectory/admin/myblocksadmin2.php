...
foreach( $group_defs as $def ) {
	if( strstr( $def , '_AM_MXDIR_ACCESSRIGHTS' ) || strstr( $def , '_AM_MXDIR_ACTIVERIGHTS' ) ) eval( $def ) ;
}
...
function list_blockinstances()
{
	...
	<form action='admin.php' name='blockadmin' method='post'>
		...
		<tr valign='middle'>
			...
			<th align='right'>"._AM_MXDIR_ACTION."</th>
		</tr>\n" ;
	...
	$module_list[_AM_MXDIR_SYSTEMLEVEL]["0-1"] = _AM_TOPPAGE;
...
function list_groups2()
{
	...
	$form = new MyXoopsGroupPermForm( _AM_MXDIR_ADGS , 1 , 'block_read' , '' ) ;
	if( $target_mid > 1 ) {
		$form->addAppendix( 'module_admin' , $target_mid , $target_mname . ' ' . _AM_MXDIR_ACTIVERIGHTS ) ;
		$form->addAppendix( 'module_read' , $target_mid , $target_mname .' ' . _AM_MXDIR_ACCESSRIGHTS ) ;
	}
